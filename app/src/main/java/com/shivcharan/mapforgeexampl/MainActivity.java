package com.shivcharan.mapforgeexampl;
...
public class MainActivity extends AppCompatActivity {
    ...
    private static final String MAP_FILE = "india.map";
    ...
    private void openMap(Uri uri) {
        try {
            ...
            if (false) {//need to handle FileInputStream in proper way
                FileInputStream fis = (FileInputStream) getContentResolver().openInputStream(uri);
                mapDataStore = new MapFile(new File(String.valueOf(fis)));
                Log.v("main","inside if uri not null");
            } else {
                File mapFile = new File(getExternalFilesDir(null), MAP_FILE);
                mapDataStore = new MapFile(mapFile);
                Log.v("main","inside if uri null");
            }
             ...
             */
            mapView.setCenter(new LatLong(30.91534501, 75.77516162));
